...
static int endpoints(libusb_device*d,struct m1522_handle*h)
{
    ...
    for(int i=0;i<c->bNumInterfaces;i++){
        ...
        for(int a=0;a<in->num_altsetting;a++){
            ...
            int rank;
            if(x->bInterfaceClass!=USB_CLASS_PRINTER||x->bInterfaceSubClass!=USB_SUBCLASS_PRINTER)continue;
            if(x->bInterfaceProtocol==USB_PROTOCOL_PRINTER_BIDIR)rank=2;
            else if(x->bInterfaceProtocol==USB_PROTOCOL_PRINTER_UNIDIR)rank=1;
            else continue;
